...
export function _PendingPurchasesTab({ suppliers }: Props) {
  ...
  const handleDelete = async (id: string) => {
    try {
      await deleteOpeningInvoice('purchases', id);
    ...
    <Card>
      <CardHeader>
        <CardTitle className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
          <span>Facturas de Compra Pendientes de Pago</span>
          <div className="flex gap-2">
            <_InvoiceImportDialog type="purchases" onImported={handleRefresh} />
      ...
      <CardContent>
        <p className="text-sm text-muted-foreground mb-4">
          Cargá las facturas de compra pendientes de pago del sistema anterior.
          Aparecerán automáticamente en el Flujo de Caja como cuentas por pagar.
        </p>
              ...
              <tbody>
                {invoices.map((inv) => (
                  <tr key={inv.id} className="border-b last:border-0">
                    <td className="py-2">{inv.supplier.businessName}</td>
                    <td className="py-2">
